...
is active and automatically attributes your time to the right project — no
timers to start, no forgetting to log a session.
...
Every byte of data this extension collects — sessions, projects, rules,
settings — stays on your device in local browser storage. There is no
account, no sign-in, and no server: the extension makes zero network
